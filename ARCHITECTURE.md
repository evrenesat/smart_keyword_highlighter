...
4. Traversal enqueues text nodes and subtree walkers; processing runs in idle-time chunks (or time-sliced fallback) to avoid long main-thread work. Mutation updates for skipped nodes (e.g., aria-hidden) are filtered before enqueue, except excluded tags still update sentence state. A mutation-storm guard pauses processing briefly when the DOM updates too rapidly, then reschedules a full rescan. The storm guard is disabled during the initial page traversal to prevent SPA boot churn from suppressing first-load highlighting.
5. Startup waits for `document.body` (via MutationObserver, DOMContentLoaded listener, and interval-based polling); a secondary observer reattaches traversal if the body is replaced. After the initial traversal completes, a startup watchdog verifies highlights were applied; if not (and the page has text), it retries with exponential backoff (up to 5 times).
